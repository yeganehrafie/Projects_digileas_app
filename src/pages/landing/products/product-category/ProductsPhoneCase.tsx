...
import CategoriesPhoneCase from "../../categories/CategoriesPhoneCase";
import axios from "axios";

const ProductsPhoneCase: React.FC = () => {
    ...
    const [currentCategorySlug, setCurrentCategorySlug] = useState<string>("قاب-گوشی");
    ...
    const categories = [
        { id: 1, name: "قاب گوشی", slug: "قاب-گوشی" },
        { id: 2, name: "قاب گوشی اندروید", slug: "قاب-گوشی-اندروید" },
        { id: 3, name: "قاب گوشی اپل", slug: "قاب-گوشی-اپل" },
    ...
    // دریافت محصولات
    const fetchProducts = useCallback(async (page: number, append: boolean = false, categorySlug: string = "قاب-گوشی") => {
    ...
    useEffect(() => {
        if (categorySlug) {
            ...
            setCurrentCategorySlug("قاب-گوشی");
            setSelectedCategories([]);
            setProducts([]);
            setCurrentPage(1);
            setHasMore(true);
            fetchProducts(1, false, "قاب-گوشی");
        }
    }, [categorySlug]);
    ...
    useEffect(() => {
        if (selectedCategories.length > 0) {
            ...
            if (currentCategorySlug !== "قاب-گوشی") {
                setCurrentCategorySlug("قاب-گوشی");
                setProducts([]);
                setCurrentPage(1);
                setHasMore(true);
    ...
    return (
        <>
            <div className="bg-white ">
                {/* BreadCrumb */}
                <BreadCrumb
                    title={'لیست محصولات قاب گوشی'}
                    items={[
                        { label: "صفحه اصلی", type: "home" },
                        { label: "محصولات قاب گوشی" }
                    ]}
                    homePath="/"
                />
...
export default ProductsPhoneCase;